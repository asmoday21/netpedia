    ...
    <div class="card shadow-sm border-0 h-100">
        ...
        <div class="card-body d-flex flex-column">
            ...
            <div class="mt-auto d-flex gap-2">
                <a href="{{ route('guru.modul.show', $modul->id) }}" class="btn btn-sm btn-outline-primary w-100">Lihat</a>
                <a href="{{ route('guru.modul.edit', $modul->id) }}" class="btn btn-sm btn-outline-warning w-100">Edit</a>
                <form action="{{ route('guru.modul.destroy', $modul->id) }}" method="POST" class="w-100" onsubmit="return confirm('Yakin ingin menghapus modul ini?');">
                    @csrf
                    @method('DELETE')
